  ...
  <div class="expert-detail">
    <div class="expert-topbar">
      <div class="layout-wrap">
        ...
        <p><span v-if="expertInfo.positionName" :title="expertInfo.positionName">{{ expertInfo.positionName | subStr(10) }}</span><span v-if="expertInfo.positionName && expertInfo.companyName"> | </span><span v-if="expertInfo.companyName" :title="expertInfo.companyName">{{ expertInfo.companyName | subStr(15) }}</span> <span v-if="expertInfo.yearsOfWorking"> {{ expertInfo.yearsOfWorking }}年</span></p>
        ...
        <span :class=" { 'btn-expert-fav': true, 'expert-faved': expertInfo.isFavorite && expertInfo.isFavorite!=-1 } "
              @click="expertFavo(expertInfo.isFavorite)">{{ expertInfo.isFavorite && expertInfo.isFavorite!=-1 ? '取消收藏' : '收藏专家' }}</span>
      </div>
    </div>
    ...
    <div class="expert-intro">
      ...
      <div class="expert-btnitem">
        ...
        <router-link :to="'/releaseResearch?expertId=' + expertInfo.id ">申请专家调研企业</router-link><a href="javascript:;"
        ...
        <div class="research-list">
          <div class="research-item"
            ...
            <router-link :to="'/companyResearch/' + item.id "
                         class="title">{{ item.title }}<span v-if="item.statusTime">{{ item.statusTime.substr(0,10) }}</span></router-link>
      ...
      <el-form :model="contactForm"
        ...
        <el-form-item prop="expertInfo"
          ...
          <div class="form-expert-info">
            <h3><span :title="expertInfo.expertName">{{ expertInfo.expertName | subStr(6) }}</span><span class="city" v-if="expertInfo.city" :title="expertInfo.city">{{ expertInfo.city | subStr(10) }}</span></h3>
            <p><span :title="expertInfo.positionName">{{ expertInfo.positionName | subStr(10) }}</span><span v-if="expertInfo.positionName && expertInfo.companyName"> | </span><span v-if="expertInfo.companyName" :title="expertInfo.companyName">{{ expertInfo.companyName | subStr(15) }}</span> <span v-if="expertInfo.yearsOfWorking"> {{ expertInfo.yearsOfWorking }}年</span></p>
        ...
        <div class="contact-info">
          ...
          <div class="dialog-btn-item">
            ...
            <el-button type="primary"
                       ...
                       @click="contactSubmit">提交</el-button>
          </div>
        </div>
      </el-form>
    </el-dialog>
        ...
        <div class="contact-info">
          ...
          <div class="dialog-btn-item">
            ...
            <el-button type="primary"
                       ...
                       @click="interviewSubmit">提交</el-button>
          </div>
        </div>
      </el-form>
    </el-dialog>
...
export default {
  ...
  , created() {
    this.$store.dispatch('getExpertDetail', this.expertId)
    this.$store.dispatch('getExpertAttribute', this.expertId)
    this.$store.dispatch('getExpertResearch', this.expertId)
    // this.getExpertInfo()
    // // 获取专家技能
  ...
  methods: {
    ...
    changeSubmitStatus(formName, time) {
      setTimeout(() => { this[`${formName}Submitting`] = false }, time || 3000)
    },
    // 关闭表单弹窗
  ...
  computed: {
    ...
    // }),
    ...mapGetters({
      expertInfo: 'getExpertDetail',
      expertAttribute: 'getExpertAttribute',
      researchList: 'getExpertResearch'
    }),
    // 发布采访单数据
    interviewForm() {
      return {
