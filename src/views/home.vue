      ...
      <div class="brief-introduction-detail w1190">
        ...
        <div class="content fn-clear">
          <div class="content-l fn-left">
            <p>聚贸云智库是聚贸旗下全球专家智库平台，智库汇聚了全球各行业各领域的顶尖专家和精英人士，为用户提供咨询、反馈信息、进行诊断、预测未来。旨在通过智库专家的经验与智慧，为社会经济等领域的发展提供满意方案或优化方案，搭建起全球企业与企业、产业与产业、国家与国家之间高效协作的桥梁。</p>
          </div>
          <div class="content-r fn-left">
            <p>Jumore Cloud Think-Tank, a sub-platform of Jumore which collects the world's top experts in various sectors of the industry, setting up bridges for global enterprises, industries and countries to provide solutions or optimizations for the development of social economic fields. </p>
          </div>
...
const fetchDetail = store => {
  return store.dispatch('getTopCarousel')
}
  ...
  export default {
    ...
    // },
      ...
      computed: {
        // ...mapGetters({
        //   carousels: 'getTopCarousel',
        //   styleInfo: 'getExpertStyle',
        //   styleBanner: 'getExpertStyleBanner',
        //   expertLists: 'getExpertList',
        //   bottomCarousels: 'getBottomCarousel'
        // }),
        ...mapState({
          carousels: state => state.home.carousels,
          styleInfo: state => state.home.styleInfo,
          styleBanner: state => state.home.styleBanner,
          expertLists: state => state.home.expertLists,
          bottomCarousels: state => state.home.bottomCarousels
        }),
        styleInfoFilter: function () {
              return this.styleInfo.slice(0, 6);
          },
    ...
    ,created() {
      //console.log(this.carousels)
      this.$store.dispatch('getTopCarousel')
      this.$store.dispatch('getExpertStyle')
      this.$store.dispatch('getExpertStyleBanner')
      this.$store.dispatch('getExpertList')
      this.$store.dispatch('getBottomCarousel')
...
<style lang="css">
  .home-wrap .carousel,.el-carousel{
      width:100%;
      height:500px;
  }
  .el-carousel{
  ...
  .expert-mask p,.expert-mask span{
    font-size:14px;
    color:#fff;
  }
